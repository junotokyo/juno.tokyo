...
async function listAllPromoCodeKeys() {
  ...
  let cursor = '0';
  do {
    const result = await kv.scan(cursor, { match: 'filmator:promo-code:*', count: 1000 });
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const codes = keys.map((key, i) => {
        ...
        return { code: key.replace('filmator:promo-code:', ''), ...data };
      });
  ...
  if (req.method === 'POST') {
    ...
    try {
      await kv.set(`filmator:promo-code:${code}`, { expires, count });
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      await kv.del(`filmator:promo-code:${code}`);
