...
# =========================================================
# 6.5 /api/popscan-* 直叩き認可（Codex Q4 指摘・middleware 迂回防止）
# =========================================================

step "/api/popscan-manage-promos 直叩き認証なし (401 期待)"
S=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/popscan-manage-promos")
assert_status "$S" "401" "直叩き middleware 通過"

step "/api/popscan-admin-stats 直叩き認証なし (401 期待)"
S=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/popscan-admin-stats")
assert_status "$S" "401" "直叩き middleware 通過"

step "/api/popscan-admin-error-log 直叩き認証なし (401 期待)"
S=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/popscan-admin-error-log")
assert_status "$S" "401" "直叩き middleware 通過"

step "/api/popscan-set-promo 直叩き認証なし (401 期待)"
S=$(curl -s -o /dev/null -w '%{http_code}' -X POST --data 'true' "$BASE/api/popscan-set-promo")
assert_status "$S" "401" "直叩き middleware 通過"
...
# /api/filmator-* 直叩き認可（Codex Q4 指摘）
step "/api/filmator-manage-promos 直叩き認証なし (401 期待)"
S=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/filmator-manage-promos")
assert_status "$S" "401" "直叩き middleware 通過"

step "/api/filmator-admin-stats 直叩き認証なし (401 期待)"
S=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/filmator-admin-stats")
assert_status "$S" "401" "直叩き middleware 通過"

step "/api/filmator-admin-error-log 直叩き認証なし (401 期待)"
S=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/filmator-admin-error-log")
assert_status "$S" "401" "直叩き middleware 通過"
