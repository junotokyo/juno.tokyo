...
// Filmator は /filmator/set-promo を持たない（docs/08 §2）＝ Promo Flag セクションなし。
...
async function refreshCodes() {
  const res = await api('/filmator/manage-promos');
  if (!res.ok) {
...
async function addCode() {
  ...
  const res = await api('/filmator/manage-promos', { method: 'POST', body: { code, expires, count } });
...
async function deleteCode(code) {
  ...
  const res = await api('/filmator/manage-promos', { method: 'DELETE', body: { code } });
  if (!res.ok) {
